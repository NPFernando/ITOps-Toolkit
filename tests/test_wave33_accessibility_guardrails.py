...
def _assert_polite_status_semantics(markdown: str) -> None:
...
def test_wave33_home_and_roadmap_keep_explicit_status_semantics():
    ...
    assert 'role="status"' in roadmap_markdown
    assert 'aria-atomic="true"' in roadmap_markdown
    assert 'tabindex="0"' in roadmap_markdown


def test_wave33_lorem_and_text_converter_keep_deterministic_outcomes():
    ...
    assert "tool-status-note-neutral" in markdown
    _assert_polite_status_semantics(markdown)
    ...
    assert 'aria-live="assertive"' in _markdown(text_app)
    assert 'aria-atomic="true"' in _markdown(text_app)
    assert 'tabindex="0"' in _markdown(text_app)
