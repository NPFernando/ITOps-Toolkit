...
PAGE = str(Path(__file__).resolve().parent.parent / "pages" / "99_Markdown_TOC_Generator.py")
...
def test_generate_shows_result():
    ...
    app.text_area[0].set_value("# Title\n\n## Section One")
    app.button[0].click().run()
    assert not app.exception

    code = " ".join(c.value for c in app.code)
    assert "[Title](#title)" in code
    md = " ".join(m.value for m in app.markdown)
    assert "tool-status-note-success" in md
    assert "Table of contents ready" in md
...
def test_empty_submission_shows_warning_status():
    ...
    assert "TOC generation needs input fixes" in md
