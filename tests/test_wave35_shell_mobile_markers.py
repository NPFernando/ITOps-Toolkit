...
def test_wave35_pages_keep_shell_and_baseline_markers():
    for rel_path in WAVE35_PAGES:
        ...
        assert 'mark_page_baseline(_baseline, "wave35-shell-mobile")' in source, f"{rel_path}: missing wave-35 marker"
...
def test_wave35_pages_keep_mobile_primary_actions_and_read_order_guidance():
    expected_snippets = {
        "app.py": [
    ...
    for rel_path, snippets in expected_snippets.items():
        ...
        for snippet in snippets:
            assert snippet in source, f"{rel_path}: missing wave-35 shell/mobile snippet {snippet!r}"

    for rel_path in WAVE35_PAGES[1:]:
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
