...
def test_wave50_status_helper_semantics_guardrails():
...
def test_wave50_read_order_and_status_live_regions_are_deterministic():
    ...
    assert "Quick tip: warning = adjust filters, success = ready to scan." in home_captions
    assert "New here? Start in Quick access, then open All tools only when needed." in home_captions
    ...
    assert "Quick tip: success = matches found, warning = adjust filters." in roadmap_captions
    assert "New here? Clear filters if results look empty, then apply one filter at a time." in roadmap_captions
    ...
    assert "Quick tip: neutral = waiting, success = ready to copy." in lorem_captions
    assert "New here? Leave seed blank for random text, then add a seed only for repeatable output." in lorem_captions
    ...
    assert "Quick tip: warning = add text, success = all outputs ready." in text_captions
    assert "New here? Start with a short word, then compare binary, hex, and octal byte order." in text_captions
