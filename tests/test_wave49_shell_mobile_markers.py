...
def test_wave49_pages_keep_shell_and_baseline_markers():
    for rel_path in WAVE49_PAGES:
        ...
        assert 'mark_page_baseline(_baseline, "wave49-shell-mobile")' in source, f"{rel_path}: missing wave-49 marker"
...
def test_wave49_pages_keep_grouped_controls_read_order_and_full_width_actions():
    expected_snippets = {
        "app.py": [
    ...
    for rel_path, snippets in expected_snippets.items():
        ...
        for snippet in snippets:
            assert snippet in source, f"{rel_path}: missing wave-49 shell/mobile snippet {snippet!r}"

    for rel_path in WAVE49_PAGES[1:]:
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
        assert "st.columns(2)" not in source, f"{rel_path}: should avoid fixed two-column form layouts on small screens"


def test_wave49_placeholder_aliases_map_to_current_tool_slugs():
    ...
    for snippet in expected_aliases:
        assert snippet in source, f"utils/ui.py: missing wave-49 placeholder alias snippet {snippet!r}"
