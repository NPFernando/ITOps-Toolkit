...
PAGE = str(Path(__file__).resolve().parent.parent / "pages" / "111_PII_Redactor.py")


def test_redact_shows_result():
    app = AppTest.from_file(PAGE, default_timeout=30)
    app.run()
    assert not app.exception

    app.text_area[0].set_value("Contact alice@example.com now")
...
def test_empty_submission_shows_warning_status():
    ...
    assert "PII redaction needs attention" in md


def test_results_persist_after_sidebar_interaction():
    app = AppTest.from_file(PAGE, default_timeout=30)
    app.run()
    app.text_area[0].set_value("alice@example.com")
    app.button[0].click().run()
    assert not app.exception
    before = len(app.code)
    assert before > 0
