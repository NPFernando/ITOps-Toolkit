...
PAGE = str(Path(__file__).resolve().parent.parent / "pages" / "121_CAA_Record_Builder.py")
...
def test_build_shows_result():
    ...
    app.text_input[0].set_value("letsencrypt.org")
    ...
    assert "Outcome: CAA record build complete" in md
...
def test_empty_state_shown_before_submit():
    ...
    assert "Outcome: CAA record builder ready" in md
...
def test_empty_submission_shows_warning_status():
    ...
    assert "Outcome: CAA record build blocked" in md
