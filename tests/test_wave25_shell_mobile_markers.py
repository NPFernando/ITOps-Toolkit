...
# Plan placeholders `pages/143_...` and `pages/144_...` are not present in this repo.
# Wave-25 mapping uses the next real existing roadmap-adjacent targets:
# 143 -> pages/139_Git_Command_Cheat_Sheet.py
# 144 -> pages/140_BIP39_Mnemonic_Generator_Validator.py
WAVE25_PAGES = (
    "app.py",
...
def test_wave25_pages_keep_shell_and_baseline_markers():
    for rel_path in WAVE25_PAGES:
...
def test_wave25_pages_keep_grouped_controls_and_full_width_actions():
    expected_snippets = {
        "app.py": [
    ...
    for rel_path, snippets in expected_snippets.items():
        ...
        for snippet in snippets:
            assert snippet in source, f"{rel_path}: missing wave-25 shell/mobile snippet {snippet!r}"

    for rel_path in WAVE25_PAGES[2:]:
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
