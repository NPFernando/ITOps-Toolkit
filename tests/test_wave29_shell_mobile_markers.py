...
def test_wave29_pages_keep_shell_and_baseline_markers():
    for rel_path in WAVE29_PAGES:
        ...
        assert 'mark_page_baseline(_baseline, "wave29-shell-mobile")' in source, f"{rel_path}: missing wave-29 marker"
...
def test_wave29_pages_use_grouped_controls_and_full_width_primary_actions():
    expected_snippets = {
        "app.py": [
    ...
    for rel_path, snippets in expected_snippets.items():
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
        assert "render_control_heading" in source, f"{rel_path}: missing wave-29 heading helper usage"
        for snippet in snippets:
            assert snippet in source, f"{rel_path}: missing wave-29 shell/mobile snippet {snippet!r}"

    for rel_path in WAVE29_PAGES[2:]:
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
