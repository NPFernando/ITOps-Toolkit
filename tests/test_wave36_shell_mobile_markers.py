...
def test_wave36_pages_keep_shell_and_baseline_markers():
    for rel_path in WAVE36_PAGES:
        ...
        assert 'mark_page_baseline(_baseline, "wave36-shell-mobile")' in source, f"{rel_path}: missing wave-36 marker"
...
def test_wave36_pages_keep_grouped_controls_and_mobile_primary_actions():
    expected_snippets = {
        "app.py": [
    ...
    for rel_path, snippets in expected_snippets.items():
        ...
        for snippet in snippets:
            assert snippet in source, f"{rel_path}: missing wave-36 shell/mobile snippet {snippet!r}"

    for rel_path in WAVE36_PAGES[1:]:
        source = (PROJECT_ROOT / rel_path).read_text(encoding="utf-8")
