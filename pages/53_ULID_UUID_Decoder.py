...
with ulid_tab:
    ...
    with tool_result_panel("ulid_decode_result", related_to="ulid_uuid_decoder"):
        ...
        if not ulid_input.strip():
            ...
            if not result["ok"]:
                render_status_note("ULID could not be decoded", result["error"], tone="warning")
            else:
                c1, c2 = st.columns(2)
...
with uuid_tab:
    ...
    with tool_result_panel("uuid_decode_result", related_to="ulid_uuid_decoder"):
        ...
        if not uuid_input.strip():
            ...
            if not result["ok"]:
                render_status_note("UUID could not be decoded", result["error"], tone="warning")
            else:
                ...
                if result["timestamp_supported"]:
                    ...
                    render_status_note(
                        ...
                        f"UUID version {result['version']} does not embed a creation timestamp.",
                        tone="neutral",
                    )
