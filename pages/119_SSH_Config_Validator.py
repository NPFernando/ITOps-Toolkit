...
if result is None:
    ...
    render_status_note("Awaiting SSH config", "Paste configuration content and run the lint check.", tone="neutral")
...
if result is not None:
    with tool_result_panel("ssh_config_validator_result_panel", related_to="ssh_config_validator"):
        ...
        if not result["ok"]:
            ...
            render_status_note(
                "Lint complete",
                "No structural issues were detected in the submitted SSH config content.",
                tone="success",
            )
        else:
