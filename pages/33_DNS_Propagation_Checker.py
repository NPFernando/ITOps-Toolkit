...
if submitted:
    ...
    if not ok:
        ...
        result = check_propagation(normalized, record_type)
        with tool_result_panel("dns_propagation_result", related_to="dns_propagation"):
            render_section_heading(f"{record_type} records across resolvers", "Same query sent directly to each public resolver.")
            if not result["ok"]:
