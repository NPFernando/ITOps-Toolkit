"""Password and passphrase generation helpers. Uses `secrets` for cryptographically secure randomness."""
...
# A small curated word list, not the full EFF diceware list -- entropy is
# reported alongside the result so users can judge it themselves.
WORDLIST = (
    "abacus", "abandon", "abstract", "acoustic", "action", "actor", "adapt", "add", "advance", "advice",
    "afraid", "again", "agree", "airport", "alarm", "album", "alert", "alien", "alley", "almost",
    "alpha", "already", "amateur", "amazing", "amber", "amount", "analyst", "anchor", "ancient", "anger",
    "angle", "animal", "ankle", "annual", "answer", "antenna", "anxiety", "apart", "apple", "approve",
    "april", "arctic", "arena", "argue", "armor", "army", "around", "arrange", "arrest", "arrive",
    "arrow", "artist", "aspect", "assault", "asset", "assist", "assume", "athlete", "atlas", "atom",
    "attack", "attend", "august", "aunt", "author", "auto", "autumn", "average", "avocado", "avoid",
    "awake", "aware", "away", "awesome", "badge", "bagel", "balance", "balcony", "bamboo", "banana",
    "banner", "barely", "bargain", "barrel", "basic", "basket", "battle", "beach", "beacon", "beam",
    "beauty", "become", "before", "begin", "behind", "believe", "belt", "bench", "benefit", "best",
    "better", "between", "beyond", "bicycle", "bike", "bind", "biology", "bird", "birth", "bitter",
    "black", "blade", "blame", "blanket", "blast", "bleak", "blend", "bless", "blind", "block",
    "blood", "blossom", "blue", "blur", "board", "boat", "body", "boil", "bomb", "bonus",
    "book", "boost", "border", "boring", "borrow", "boss", "bottom", "bounce", "boundary", "bracket",
    "brain", "branch", "brand", "brass", "brave", "bread", "break", "breeze", "brick", "bridge",
    "brief", "bright", "bring", "broad", "broccoli", "broken", "bronze", "broom", "brother", "brown",
    "brush", "bubble", "buddy", "budget", "buffalo", "build", "bulb", "bulk", "bullet", "bundle",
    "bunker", "burden", "burger", "burst", "bus", "business", "busy", "butter", "buyer", "cabin",
    "cable", "cactus", "cage", "cake", "calm", "camera", "camp", "canal", "cancel", "candy",
    "cannon", "canoe", "canvas", "canyon", "capable", "capital", "captain", "carbon", "card", "cargo",
    "carpet", "carry", "cart", "case", "cash", "castle", "casual", "catalog", "catch", "category",
    "cattle", "cause", "caution", "cave", "ceiling", "celery", "cement", "census", "century", "cereal",
    "certain", "chair", "chalk", "champion", "change", "chaos", "chapter", "charge", "chase", "cheap",
    "check", "cheese", "cherry", "chest", "chicken", "chief", "child", "chimney", "choice", "choose",
    "chronic", "chuckle", "chunk", "cigar", "circle", "citizen", "city", "civil", "claim", "clarify",
    "class", "claw", "clay", "clean", "clerk", "clever", "click", "client", "cliff", "climb",
    "clinic", "clip", "clock", "close", "cloth", "cloud", "clown", "club", "clump", "cluster",
    "coach", "coast", "coconut", "code", "coffee", "coin", "collect", "color", "column", "combine",
    "come", "comfort", "comic", "common", "compass", "concert", "conduct", "confirm", "connect", "consider",
    ...
    "cruise", "crumble", "crush", "crystal", "cube", "culture", "cup", "curious", "current", "curtain",
    "curve", "cushion", "custom", "cycle", "dance", "danger", "daring", "dash", "dawn", "daylight",
    "deal", "debate", "debris", "decade", "decide", "decline", "decorate", "decrease", "deer", "defense",
    ...
    "dumb", "dune", "during", "dust", "dutch", "duty", "dwarf", "eager", "eagle", "early",
    "earn", "earth", "easily", "east", "easy", "echo", "ecology", "economy", "edge", "edit",
    "educate", "effort", "eight", "either", "elbow", "elder", "electric", "elegant", "element", "elephant",
    "elevator", "elite", "else", "embark", "embody", "emerge", "emotion", "empower", "empty", "enable",
    ...
    "equal", "equip", "erase", "erode", "erosion", "error", "erupt", "escape", "essay", "essence",
    ...
    "fetch", "fever", "fiber", "fiction", "field", "figure", "file", "film", "filter", "final",
    ...
    "genius", "gentle", "genuine", "gesture", "ghost", "giant", "gift", "giggle", "ginger", "giraffe",
    ...
    "luxury", "lyrics", "machine", "magic", "magnet", "maid", "mail", "main", "major", "make",
    ...
    "mixed", "mixture", "mobile", "model", "modify", "moment", "monitor", "monkey", "monster", "month",
    ...
    "oval", "oven", "over", "owner", "oxygen", "oyster", "ozone", "pact", "paddle", "page",
...
def generate_password(
    ...
    """Generate a random password of ``length`` characters from the selected character sets."""
    result: dict[str, Any] = {"ok": False, "password": None, "entropy_bits": None, "pool_size": None, "error": None}
    if not (MIN_LENGTH <= length <= MAX_LENGTH):
        result["error"] = f"Length must be between {MIN_LENGTH} and {MAX_LENGTH}."
        return result
    ...
    if not pool:
        result["error"] = "Select at least one character type."
        return result

    password = "".join(secrets.choice(pool) for _ in range(length))
    entropy_bits = round(length * math.log2(len(pool)), 1)
    result.update({"ok": True, "password": password, "entropy_bits": entropy_bits, "pool_size": len(pool)})
    return result
...
def generate_passphrase(
    ...
    """Generate a diceware-style passphrase of ``word_count`` words from WORDLIST."""
    result: dict[str, Any] = {"ok": False, "passphrase": None, "entropy_bits": None, "error": None}
    if not (MIN_WORDS <= word_count <= MAX_WORDS):
        result["error"] = f"Word count must be between {MIN_WORDS} and {MAX_WORDS}."
        return result
    ...
    entropy_bits = word_count * math.log2(len(WORDLIST))
    if include_number:
        number = secrets.randbelow(90) + 10
        words.append(str(number))
        entropy_bits += math.log2(90)

    passphrase = (separator or "").join(words)
    result.update({"ok": True, "passphrase": passphrase, "entropy_bits": round(entropy_bits, 1)})
    return result
