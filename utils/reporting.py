...
def build_domain_health_psa_note(
    domain: str,
    dns_summary: dict[str, Any],
    ssl_result: dict[str, Any],
    http_result: dict[str, Any],
    ...
    """Build a plain-text ticket-note summary with no markdown syntax.

    PSA ticket note fields (ConnectWise Manage, Autotask, Halo, ...) commonly
    render as plain text -- markdown symbols like # and ** show up literally
    rather than formatting, so this is deliberately not the same as the
    Markdown export.
    """
    timestamp = generated_at or datetime.now(UTC)
    posture = dns_summary.get("email_security_posture", {})
    ...
    lines = [
        f"Domain Health Summary - {_value(domain)}",
        f"Generated: {_value(timestamp)}",
        "",
        f"Risk score: {_value(risk.get('score'))} ({_value(risk.get('status'))})",
        f"DNS status: {_value(dns_summary.get('status'))}",
        f"Email security status: {_value(dns_summary.get('email_status'))}",
        f"SSL status: {_value(ssl_result.get('tls_status'))} ({_value(ssl_result.get('days_remaining'))} days remaining)",
        f"HTTP status: {_value(http_result.get('status_code'))} ({_value(http_result.get('response_time_ms'))} ms)",
        "",
        "Recommendations:",
    ]
    if recommendations:
        ...
        lines.append("- No major recommendations from the current checks.")
    lines.extend(["", "Generated by ITOps Toolkit. Public, one-time checks -- nothing is stored."])
    return "\n".join(lines)


def build_log_analysis_psa_note(findings: list[dict[str, Any]], generated_at: datetime | None = None) -> str:
    """Build a plain-text ticket-note summary of rule-based log findings."""
    timestamp = generated_at or datetime.now(UTC)
    lines = [
    ...
    for item in findings or []:
        lines.append(f"[{_value(item.get('severity'))}] {_value(item.get('likely_issue'))}")
        lines.append(f"Cause: {_value(item.get('possible_cause'))}")
        commands = item.get("commands_to_check") or []
        if commands:
            lines.append("Commands to check:")
            lines.extend(f"  - {command}" for command in commands)
        steps = item.get("safe_next_steps") or []
        if steps:
            lines.append("Next steps:")
            lines.extend(f"  - {step}" for step in steps)
        lines.append("")
    lines.append("Generated by ITOps Toolkit. Public, one-time checks -- nothing is stored.")
    return "\n".join(lines)


INCIDENT_MESSAGE_TARGETS: tuple[str, ...] = ("slack", "teams")
_INCIDENT_STATUS_EMOJI = {
...
def _incident_bold(text: str, target: str) -> str:
    # Slack mrkdwn uses single-asterisk bold; Teams chat/GFM-style markdown uses double.
    return f"*{text}*" if target == "slack" else f"**{text}**"


def _incident_status_emoji(status: Any) -> str:
    return _INCIDENT_STATUS_EMOJI.get(_value(status, "").lower(), ":grey_question:")


def build_domain_health_incident_message(
    domain: str,
    dns_summary: dict[str, Any],
    ssl_result: dict[str, Any],
    http_result: dict[str, Any],
    ...
    """Build a ready-to-paste Slack/Teams incident-update message from Domain Health results."""
    if target not in INCIDENT_MESSAGE_TARGETS:
        return {"ok": False, "message": "", "error": f"Unknown target. Choose one of: {', '.join(INCIDENT_MESSAGE_TARGETS)}."}
    ...
    lines = [
        f"{_incident_status_emoji(status)} {_incident_bold(f'Domain Health: {_value(domain)}', target)}",
        f"Risk score: {_value(risk.get('score'))} ({_value(status)})",
        f"DNS: {_value(dns_summary.get('status'))} | Email security: {_value(dns_summary.get('email_status'))}",
        f"SSL: {_value(ssl_result.get('tls_status'))} ({_value(ssl_result.get('days_remaining'))} days remaining)",
        f"HTTP: {_value(http_result.get('status_code'))} ({_value(http_result.get('response_time_ms'))} ms)",
    ]
    if recommendations:
        lines.append(_incident_bold("Recommendations:", target))
        lines.extend(f"- {item}" for item in recommendations)

    return {"ok": True, "message": "\n".join(lines), "error": None}


def build_uptime_incident_message(url: str, trend: dict[str, Any], target: str = "slack") -> dict[str, Any]:
    """Build a ready-to-paste Slack/Teams incident-update message from an Uptime Trend run."""
    if target not in INCIDENT_MESSAGE_TARGETS:
        return {"ok": False, "message": "", "error": f"Unknown target. Choose one of: {', '.join(INCIDENT_MESSAGE_TARGETS)}."}

    uptime_pct = trend.get("uptime_pct")
    status = "Healthy" if uptime_pct == 100 else "Warning" if (uptime_pct or 0) >= 50 else "Critical"
    samples = trend.get("samples") or []
    failures = [s for s in samples if not s.get("ok")]

    lines = [
        f"{_incident_status_emoji(status)} {_incident_bold(f'Uptime Trend: {_value(url)}', target)}",
        f"Uptime: {_value(uptime_pct)}% over {len(samples)} check(s)",
        f"Latency: avg {_value(trend.get('avg_latency_ms'))} ms, min {_value(trend.get('min_latency_ms'))} ms, max {_value(trend.get('max_latency_ms'))} ms",
    ]
    if failures:
        lines.append(_incident_bold(f"{len(failures)} failed check(s):", target))
        lines.extend(f"- Check {s.get('index')}: {_value(s.get('error'))}" for s in failures)
